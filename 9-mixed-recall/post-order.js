...
// Approach
// DFS traversal
// When a node has children, traverse children first (left, then right)
// When children have been traversed, add current node's value to output array

// Time complexity: O(n) linear
// Space complexity: O(n) linear
const postOrder = (root, values = []) => {
  if (!root) return [];
  if (root.left) postOrder(root.left, values);
  if (root.right) postOrder(root.right, values);
